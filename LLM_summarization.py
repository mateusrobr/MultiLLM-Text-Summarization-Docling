...
def evaluate_responses (responses, question):
    ...
    evaluation_prompt = f""" Você é um avaliador especializado avaliar respostas para perguntas sobre editais de Processo Seletivo da UFPA. Aqui esta uma pergunta {question}, avalie qual é a melhor resposta entre as respostas geradas pelos modelos: {', '.join([f'Response {i + 1}: {response}' for i, response in enumerate(responses)])}. Escolha uma das respostas como a melhor resposta e mostre. Não invente nem acrescente informações, apenas escolha a melhor resposta e mostre-a."""
